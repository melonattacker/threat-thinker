...
def export_md(
    ...
    for t in threats:
        ...
        lines.append(
            "| {id} | {sev} | {title} | {why} | {aff} | {stride} | {refs} | {ev} | {score} |".format(
                id=t.id,
                sev=t.severity,
...
def diff_reports(
    ...
    removed_nodes = [n for n in before_graph.get("nodes", []) if n["id"] in removed_node_ids]
    ...
    added_edges = [e for e in after_graph.get("edges", []) if edge_key(e) in added_edge_keys]
    removed_edges = [e for e in before_graph.get("edges", []) if edge_key(e) in removed_edge_keys]
    ...
    diff_summary = {
        "graph_changes": {
            ...
            "added_nodes": [{"id": n["id"], "label": n["label"], "type": n.get("type", "")} for n in added_nodes],
            "removed_nodes": [{"id": n["id"], "label": n["label"], "type": n.get("type", "")} for n in removed_nodes],
            "added_edges": [{"src": e["src"], "dst": e["dst"], "label": e.get("label", "")} for e in added_edges],
            "removed_edges": [{"src": e["src"], "dst": e["dst"], "label": e.get("label", "")} for e in removed_edges],
        },
        "threat_changes": {
            "threats_added": len(added_threats),
            "threats_removed": len(removed_threats),
            "added_threats": [{"id": t["id"], "title": t["title"], "severity": t["severity"]} for t in added_threats],
            "removed_threats": [{"id": t["id"], "title": t["title"], "severity": t["severity"]} for t in removed_threats],
        }
    }
...
def export_diff_md(diff_data: Dict, out_path: Optional[str] = None) -> str:
    ...
    if graph_changes.get("nodes_added"):
        ...
        for node in graph_changes["nodes_added"]:
            lines.append(f"- **{node['id']}** ({node.get('type', '')}) - {node['label']}")
        lines.append("")
    ...
    if threat_changes.get("added"):
        lines.append("## Added Threats")
        lines.append("")
        lines.append("| ID | Severity | Title | Why |")
        lines.append("|---|---|---|---|")
        for threat in threat_changes["added"]:
            lines.append(f"| {threat['id']} | {threat['severity']} | {threat['title'].replace('|', '/')} | {threat.get('why', '').replace('|', '/')} |")
        lines.append("")
    
    if threat_changes.get("removed"):
        lines.append("## Removed Threats")
        lines.append("")
        lines.append("| ID | Severity | Title | Why |")
        lines.append("|---|---|---|---|")
