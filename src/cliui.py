...
class ProgressBar:
    ...
    def _draw(self):
        ...
        sys.stdout.write(f"\r{Colors.CYAN}[{bar}]{Colors.RESET} {percent:6.1f}% ({self.current}/{self.total}) {elapsed:.1f}s")
        sys.stdout.flush()
...
class ThinkingIndicator:
    ...
    def _animate(self):
        ...
        while self.is_running:
            ...
            sys.stdout.write(f"\r{Colors.YELLOW}{frame} {self.message}...{Colors.RESET}")
            sys.stdout.flush()
...
"""
    ...
    def show_summary(self, threats_count: int, processing_time: float):
        ...
        print(f"  {Colors.CYAN}•{Colors.RESET} Identified {Colors.BOLD}{threats_count}{Colors.RESET} threats")
        print(f"  {Colors.CYAN}•{Colors.RESET} Processing time: {Colors.BOLD}{processing_time:.1f}s{Colors.RESET}")
    ...
    def show_metrics_summary(self, metrics: Dict[str, Any]):
        ...
        if hasattr(metrics, 'total_lines'):
            ...
            if total_lines > 10000:  # Likely file size in bytes
                ...
                print(f"  {Colors.CYAN}•{Colors.RESET} Processed {Colors.BOLD}{total_lines}{Colors.RESET} lines")
            
            # Show parsing success rates if available
            if hasattr(metrics, 'nodes_parsed'):
                print(f"  {Colors.CYAN}•{Colors.RESET} Found {Colors.BOLD}{metrics.nodes_parsed}{Colors.RESET} nodes")
            if hasattr(metrics, 'edges_parsed'):
                print(f"  {Colors.CYAN}•{Colors.RESET} Found {Colors.BOLD}{metrics.edges_parsed}{Colors.RESET} edges")
            if hasattr(metrics, 'import_success_rate'):
                ...
                print(f"  {Colors.CYAN}•{Colors.RESET} Success rate: {color}{Colors.BOLD}{rate:.1f}%{Colors.RESET}")
            ...
            if 'total_lines' in metrics:
                ...
                if lines > 10000:
                    ...
                    print(f"  {Colors.CYAN}•{Colors.RESET} Processed {Colors.BOLD}{lines}{Colors.RESET} lines")
        else:
    ...
    def create_thinking_indicator(self, message: str = "AI is analyzing") -> ThinkingIndicator:
    ...
    def show_threats_preview(self, threats: List[Any], max_show: int = 3):
        ...
        self.info(f"Preview of identified threats (showing {min(len(threats), max_show)} of {len(threats)}):")
        
        for i, threat in enumerate(threats[:max_show]):
            severity_color = self._get_severity_color(threat.severity)
            print(f"  {Colors.BOLD}{i+1}.{Colors.RESET} {severity_color}{threat.severity}{Colors.RESET} - {threat.title}")
