...
from src.parsers.image_parser import parse_image, _encode_image_to_base64, _analyze_image_with_llm, _parse_llm_response_to_graph
from src.models import Graph, Node, Edge, ImportMetrics
...
class TestImageParser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.png_data = base64.b64decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==')
    ...
    def test_parse_llm_response_to_graph(self):
        ...
        llm_data = {
            "nodes": [
                ...
                {"id": "web_server", "label": "Web Server", "type": "service", "zone": "dmz"},
                {"id": "database", "label": "Database", "type": "database", "zone": "internal"}
            ],
            "edges": [
                {"src": "user", "dst": "web_server", "label": "HTTPS Request", "protocol": "HTTPS"},
                {"src": "web_server", "dst": "database", "label": "SQL Query", "protocol": "TCP"}
            ]
        }
    ...
    def test_parse_llm_response_with_invalid_data(self):
        ...
        llm_data = {
            ...
            "edges": [
                ...
                {"src": "valid_node", "dst": "valid_node", "label": "Self loop"}  # Valid edge
            ]
        }
    ...
    def test_analyze_image_with_llm_success(self, mock_llm_client_class):
        ...
        mock_response = json.dumps({
            ...
            "edges": [{"src": "test_node", "dst": "test_node", "label": "Self connection"}]
        })
    ...
    def test_parse_image_integration(self, mock_analyze):
        ...
        mock_analyze.return_value = {
            ...
            "edges": [
                {"src": "frontend", "dst": "backend", "label": "API Call", "protocol": "HTTPS"}
            ]
        }
